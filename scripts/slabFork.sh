...
echo "\"block.colouredstuff.${seed}_black\" : \"Black Stonebrick Slab\","
...
do
  ...
  # echo "$color"
   ...
   # blockstate      
	...
	# block model  
	cp "${folder}"/models/block/slab/"${input}"_black.json "${folder}"/models/block/slab/"${input}"_"${color}".json
	sed -i -e "s/black/${color}/g" "${folder}"/models/block/slab/"${input}"_"${color}".json
	cp "${folder}"/models/block/slab/"${input}"_black_upper.json "${folder}"/models/block/slab/"${input}"_"${color}"_upper.json
	sed -i -e "s/black/${color}/g" "${folder}"/models/block/slab/"${input}"_"${color}"_upper.json
   ...
   #recipe
	...
	echo "\"block.colouredstuff.${seed}_${color}\" : \"${name} Stonebrick Slab\","
