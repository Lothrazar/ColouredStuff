...
echo "\"block.colouredstuff.${seed}_black\" : \"Black Wooden Fence\","
...
do
  ...
  # echo "$color"
   ...
   # blockstate      
	...
	sed -i -e "s/black/${color}/g" "${folder}"/models/block/fence/"${input}"_"${color}"_post.json
	cp "${folder}"/models/block/fence/"${input}"_black_side.json "${folder}"/models/block/fence/"${input}"_"${color}"_side.json
	sed -i -e "s/black/${color}/g" "${folder}"/models/block/fence/"${input}"_"${color}"_side.json
