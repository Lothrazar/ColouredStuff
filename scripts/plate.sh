...
echo "\"block.colouredstuff.${seed}_black\" : \"Black Wooden Pressure Plate\","
...
do
  ...
  # echo "$color"
   ...
   # blockstate      
	...
	# block model  
	cp "${folder}"/models/block/pressure_plate/"${input}"_black.json "${folder}"/models/block/pressure_plate/"${input}"_"${color}".json
	sed -i -e "s/black/${color}/g" "${folder}"/models/block/pressure_plate/"${input}"_"${color}".json
	cp "${folder}"/models/block/pressure_plate/"${input}"_black_down.json "${folder}"/models/block/pressure_plate/"${input}"_"${color}"_down.json
	sed -i -e "s/black/${color}/g" "${folder}"/models/block/pressure_plate/"${input}"_"${color}"_down.json
   ...
   #recipe
	...
	echo "\"block.colouredstuff.${seed}_${color}\" : \"${name} Wooden Pressure Plate\","
