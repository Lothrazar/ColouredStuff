...
echo "\"block.colouredstuff.${seed}_black\" : \"Black Wooden Gate\","
...
do
  ...
  # echo "$color"
   ...
   # blockstate      
	...
	# block model   
	cp "${folder}"/models/block/gate/"${input}"_black.json "${folder}"/models/block/gate/"${input}"_"${color}".json
	sed -i -e "s/black/${color}/g" "${folder}"/models/block/gate/"${input}"_"${color}".json
	cp "${folder}"/models/block/gate/"${input}"_black_open.json "${folder}"/models/block/gate/"${input}"_"${color}"_open.json
	sed -i -e "s/black/${color}/g" "${folder}"/models/block/gate/"${input}"_"${color}"_open.json
	cp "${folder}"/models/block/gate/"${input}"_black_wall.json "${folder}"/models/block/gate/"${input}"_"${color}"_wall.json
	sed -i -e "s/black/${color}/g" "${folder}"/models/block/gate/"${input}"_"${color}"_wall.json
	cp "${folder}"/models/block/gate/"${input}"_black_wall_open.json "${folder}"/models/block/gate/"${input}"_"${color}"_wall_open.json
	sed -i -e "s/black/${color}/g" "${folder}"/models/block/gate/"${input}"_"${color}"_wall_open.json
	...
	echo "\"block.colouredstuff.${seed}_${color}\" : \"${name} Wooden Gate\","
